...
class InputBase(object):
    ...
    def __init__(self, pin_name, pull=None, convert_func=None):
        ...
        self._pin_name = int(name_parts[0].strip())
...
class Digital(DigitalBase):
    ...
    def __init__(self, pin_name, switch_func=None, hl_func=None, lh_func=None, **kwargs):
        '''Arguments not in the inheritance chain:
        switch_func: if a function is provided, it is called when the pin
            transitions from a low state (0) to a high state (1) or vice versa.
            Is given one argument, the input name.
        ...
        if switch_func is not None and (hl_func is not None or lh_func is not None):
            raise ValueError("Cannot specify both switch_func and (hl_func or lh_func).")

        if switch_func is None:
            self.hl_func = hl_func
            self.lh_func = lh_func
        else:
            self.hl_func = switch_func
            self.lh_func = switch_func
    ...
    def service_input(self):
        ...
        if self._new_val > self._cur_val:
            ...
            if self.lh_func:
                self.lh_func(self.input_name)
        else:
            # transition from high to low occurred.
            if self.hl_func:
                self.hl_func(self.input_name)
